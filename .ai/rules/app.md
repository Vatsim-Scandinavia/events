...
Rosters belong to one event occurrence and keep UTC slot/availability times within that occurrence; the event schedule is immutable once a roster exists. Opened rosters are visible to any user with a Controller role in any FIR, separately from private event visibility; coordinator management still follows event owner/accepted-collaborator access. Roster participation deliberately bypasses the global Administrator shortcut and requires an actual Controller role, an open roster and an available occurrence. Preserve booked slot identities/times and positions with submitted interest when editing. RosterMutation acquires an event write lock before transactional validation (including SQLite); booking mutations acquire the controller user write lock before the event lock to prevent overlapping bookings across events. Audit roster writes in the same transaction.
