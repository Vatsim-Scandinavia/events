...
## Event publication and public read boundaries
Only owner FIR coordinators (or Administrators) publish or unpublish events, using the event write lock and transactional published/unpublished audits. Public visibility requires published_at plus published/cancelled status. Cancelling a published event keeps its public cancellation notice; restoring returns a private draft and clears published_at. Unpublishing removes guest access immediately, including banners, while retaining cancellation state if cancelled. Use one events.index/show/banner route set at /events and /events/{event}: guests see public events; signed-in users additionally see private events allowed by existing event view permissions. The same pages conditionally receive staff controls and collaboration/roster metadata only after per-event policy authorization. Keep Event::visibleTo and EventPolicy::view/viewAny scoped to private staff access because roster visibility relies on them. Creation, editing and every staff mutation remain behind authentication and policies/gates. Published event edits become public immediately.
