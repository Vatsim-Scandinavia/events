...
Roster configuration belongs once to each event; local wall times and day offsets are projected through its timezone for every occurrence. Bookings and interest remain dated snapshots, never carried forward; default roster views advance to the current/next scheduled occurrence without GET writes. Protect future commitments during template edits and retain historical snapshots when positions change. Opened rosters are visible to any Controller role in any FIR; event management remains owner/accepted-collaborator scoped and participation requires an actual Controller role even for Administrators. All mutations target an explicit occurrence_date where applicable; return_to_current only affects the redirect. Serialize writes with an event write lock (also SQLite), and controller user lock before event for bookings. Audit in the same transaction; event schedules remain locked once a roster exists.
